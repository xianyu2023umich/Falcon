module ModSpherical
    ...
    function ModSpherical_abs(nr,nt,np,ng,A) result(f)
        implicit none
        integer,intent(in)          ::  nr,nt,np,ng
        real(8),intent(in)          ::  A(-ng+1:nr+ng,-ng+1:nt+ng,-ng+1:np+ng,3)
        real(8)                     ::  f(-ng+1:nr+ng,-ng+1:nt+ng,-ng+1:np+ng)

        f=sqrt(A(:,:,:,1)**2+A(:,:,:,2)**2+A(:,:,:,3)**2)
    end function ModSpherical_abs

    function ModSpherical_dot(nr,nt,np,ng,A,B) result(f)
        implicit none
        integer,intent(in)          ::  nr,nt,np,ng
        real(8),intent(in)          ::  A(-ng+1:nr+ng,-ng+1:nt+ng,-ng+1:np+ng,3),&
    ...
    function ModSpherical_cross(nr,nt,np,ng,A,B) result(C)
        ...
        real(8),intent(in)          ::  A(-ng+1:nr+ng,-ng+1:nt+ng,-ng+1:np+ng,3),&
                                        B(-ng+1:nr+ng,-ng+1:nt+ng,-ng+1:np+ng,3)
        real(8)                     ::  C(-ng+1:nr+ng,-ng+1:nt+ng,-ng+1:np+ng,3)
    ...
    function ModSpherical_curl(nr,nt,np,ng,r,t,dr,dt,dp,A) result(curl_A)
        implicit none
        integer,intent(in)          ::  nr,nt,np,ng
        real(8),intent(in)          ::  r(-ng+1:ng+nr),t(-ng+1:ng+nt)
        real(8),intent(in)          ::  dr,dt,dp
        real(8),intent(in)          ::  A(-ng+1:ng+nr,-ng+1:ng+nt,-ng+1:ng+np,1:3)
        real(8)                     ::  At_r(-ng+1:ng+nr,-ng+1:ng+nt,-ng+1:ng+np)
        real(8)                     ::  Ap_r(-ng+1:ng+nr,-ng+1:ng+nt,-ng+1:ng+np)
        real(8)                     ::  Ar_sint(-ng+1:ng+nr,-ng+1:ng+nt,-ng+1:ng+np)
        real(8)                     ::  Ap_sint(-ng+1:ng+nr,-ng+1:ng+nt,-ng+1:ng+np)
        real(8)                     ::  sint(-ng+1:ng+nt),r_inverse(1:nr),sint_inverse(1:nt)
        real(8)                     ::  curl_A(1:nr,1:nt,1:np,1:3)
    ...
    function ModSpherical_div(nr,nt,np,ng,r,t,dr,dt,dp,A) result(div_A)
        ...
        real(8),intent(in)          ::  dr,dt,dp
        real(8),intent(in)          ::  A(-ng+1:ng+nr,-ng+1:ng+nt,-ng+1:ng+np,1:3)
        real(8)                     ::  A_scaled(-ng+1:ng+nr,-ng+1:ng+nt,-ng+1:ng+np,1:2)
        real(8)                     ::  r2(-ng+1:ng+nr),sint(-ng+1:ng+nt),&
                                        r2_inverse(1:nr),r_inverse(1:nr),sint_inverse(1:nt)
        real(8)                     ::  div_A(1:nr,1:nt,1:np),div_A23(1:nr,1:nt,1:np)
        integer                     ::  ir,it,ip
    ...
    function ModSpherical_Grad_f_O2(nr,nt,np,ng,r,t,dr,dt,dp,f) result(Grad_f)
        implicit none
        integer,intent(in)          ::  nr,nt,np,ng
        real(8),intent(in)          ::  r(-ng+1:ng+nr),&
                                        t(-ng+1:ng+nt)
        real(8),intent(in)          ::  dr,dt,dp
        real(8),intent(in)          ::  f(-ng+1:ng+nr,-ng+1:ng+nt,-ng+1:ng+np)
        real(8)                     ::  Grad_f(1:nr,1:nt,1:np,1:3)
        real(8)                     ::  r_inverse(1:nr),sint_inverse(1:nt)
        integer                     ::  ir,it,ip
        ...
        ! r term
        Grad_f(:,:,:,1)=ModDerivative_1st_O2_3D(f,nr,nt,np,ng,dr,dt,dp,1)

        ! th term
        Grad_f(:,:,:,2)=ModDerivative_1st_O2_3D(f,nr,nt,np,ng,dr,dt,dp,2)
        do ip=1,np
            do it=1,nt
        ...
        ! ph term
        Grad_f(:,:,:,3)=ModDerivative_1st_O2_3D(f,nr,nt,np,ng,dr,dt,dp,3)
        do ip=1,np
            do it=1,nt
    ...
    end function ModSpherical_Grad_f_O2
    ...
    function ModSpherical_A_dot_Grad_f(nr,nt,np,ng,r,t,dr,dt,dp,A,f) result(A_dot_Grad_f)
        ...
        real(8),intent(in)          ::  A(-ng+1:ng+nr,-ng+1:ng+nt,-ng+1:ng+np,1:3)
        real(8),intent(in)          ::  f(-ng+1:ng+nr,-ng+1:ng+nt,-ng+1:ng+np)
        real(8)                     ::  A_dot_Grad_f(1:nr,1:nt,1:np),tmp(1:nr,1:nt,1:np)
        real(8)                     ::  r_inverse(1:nr),sint_inverse(1:nt)
        integer                     ::  ir,it,ip
    ...
    function ModSpherical_A_dot_nabla_B(nr,nt,np,ng,r,t,dr,dt,dp,A,B) result(A_dot_nabla_B)
        ...
        real(8),intent(in)          ::  dr,dt,dp
        real(8),intent(in)          ::  A(-ng+1:ng+nr,-ng+1:ng+nt,-ng+1:ng+np,1:3)
        real(8),intent(in)          ::  B(-ng+1:ng+nr,-ng+1:ng+nt,-ng+1:ng+np,1:3)
        real(8)                     ::  A_dot_nabla_B(1:nr,1:nt,1:np,1:3)
        real(8)                     ::  tmp(1:nr,1:nt,1:np)
        real(8)                     ::  cotant(1:nt),r_inverse(1:nr)
