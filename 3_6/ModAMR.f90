    ...
    recursive subroutine ModAMR_Divide_OneBranch(Tree,Node,iAMR)
        ...
        if (Node%if_leaf) then
            ...
            if (AMR1%if_global) then
                if (ModMath_IfLinesInterSect(Node%rtp_range(1,:),AMR1%rtp_range_Rsun(1,:)*R_sun__CGS)) &
                    call YinYangTree_Divide(Tree,Node,AMR1%rtp_if_divide(1:3))
            else
                if (Node%if_yin) then
                    if (ModMath_IfLinesInterSect(Node%rtp_range(1,:),AMR1%rtp_range_Rsun(1,:)*R_sun__CGS) .and. &
                        ModMath_IfLinesInterSect(Node%rtp_range(2,:),AMR1%rtp_range_Rsun(2,:)) .and. &
                        ModMath_IfLinesInterSect(Node%rtp_range(3,:),AMR1%rtp_range_Rsun(3,:))) &
                        call YinYangTree_Divide(Tree,Node,AMR1%rtp_if_divide(1:3))
                end if
            end if
        else
