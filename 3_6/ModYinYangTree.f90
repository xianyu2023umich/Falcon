module ModYinYangTree
    ...
    use ModAllocation,      only:   ModAllocation_Init,&
                                    ...
                                    nLocalLeafNodes,&
                                    iLocalLeafNodes
    use ModStratification_new, only: ModStratification_new_get_middle_r

    ! one node
    type TreeNode
        logical                     ::  if_leaf                 ! if this is the leaf node.
        logical                     ::  if_yin                  ! If yin
        real(8)                     ::  rtp_range(3,2)          ! the coordinate ranges for three directions.
    ...
    type GC_interface
        ...
        real(8),allocatable             ::  message(:,:)            ! the message consist of all the messages for all the block pairs
    ...
    type YYTree
        ...
        real(8)                         ::  r_range_CGS(2)          ! r range of tree in CGS
    ...
    subroutine YinYangTree_InitTree(Tree,r_range_CGS)
        implicit none
        type(YYTree)                ::  Tree                ! the generated tree
        real(8),intent(in)          ::  r_range_CGS(2)      ! radial range in CGS
        real(8)                     ::  rtp_range_root_CGS(3,2) ! rtp_range for yin/yang root in CGS
        ...
        Tree%r_range_CGS=r_range_CGS
        rtp_range_root_CGS(1,:)=r_range_CGS
        rtp_range_root_CGS(2,:)=[dpi*0.25,dpi*0.75]
        rtp_range_root_CGS(3,:)=[-dpi*0.75,dpi*0.75]

        ! Initialize the Yin/Yang root
        call YinYangTree_InitNode(Tree%Yin, rtp_range_root_CGS,.true. )
        call YinYangTree_InitNode(Tree%Yang,rtp_range_root_CGS,.false.)
    ...
    subroutine YinYangTree_InitNode(Node,rtp_range,if_yin)
        ...
        real(8),intent(in)          ::  rtp_range(3,2)      ! r th ph range
    ...
    subroutine YinYangTree_Divide_r(Tree,Node,i_option)
        ...
        real(8)                     ::  r_middle
        real(8)                     ::  rtp_range1(3,2)     ! rtp range of child
    ...
    subroutine YinYangTree_Divide(Tree,Node,rtp_if_divide)
        ...
        real(8)                     ::  rtp_range(3,2)      ! rtp range of Node
        real(8)                     ::  rtp_range1(3,2)     ! rtp range of child
    ...
    subroutine YinYangTree_if_top_bottom(Block1,r_range_CGS)
        implicit none
        type(BlockType),intent(inout)   ::  Block1              ! The input Block
        real(8),intent(in)              ::  r_range_CGS(2)      ! r_range of whole tree in CGS
        
        if (Block1%xi_I(ni+ng) .gt. r_range_CGS(2)) then
            Block1%if_top=.True.
        else
            Block1%if_top=.False.
        end if
        if (Block1%xi_I(-ng+1) .lt. r_range_CGS(1)) then
    ...
    subroutine YinYangTree_SetAll(Tree)
        implicit none
        type(YYTree)                ::  Tree                ! Tree
        real(8),allocatable         ::  rtp_ranges(:,:,:)   ! of local blocks
        logical                     ::  if_yin
    ...
    recursive subroutine YinYangTree_GetLocalBlocksInfo(Tree,Node,xijk_ranges,iBlockLocal_Current)
        ...
        type(YYTree)                :: Tree
        type(TreeNode),target       :: Node
        real(8),intent(inout)       :: xijk_ranges(3,2,Tree%nLocalBlocks)
    ...
    function YinYangTree_IfInsideNode(Node,rtp,if_yin) result(if_inside)
        ...
        real(8),intent(in)          ::  rtp(3)              ! the point
        real(8)                     ::  rtp_here(3)         ! rtp for yy of node
    ...
    function YinYangTree_FindNode(Tree,rtp,if_yin) result(Node1)
        implicit none
        type(YYTree)                ::  Tree                ! The tree
        real(8),intent(in)          ::  rtp(3)              ! rtp of point
    ...
    recursive function YinYangTree_FindNode_OneBranch(Node0,rtp,if_yin) result(Node1)
        ...
        real(8),intent(in)          ::  rtp(3)              ! Point coord
        real(8)                     ::  rtp_here(3)         ! rtp here
    ...
    function YinYangTree_Get_rtp_range(Tree,iLeafNode) result(rtp_range)
        ...
        real(8)                     ::  rtp_range(3,2)      ! The output
    ...
    recursive subroutine YinYangTree_Get_rtp_range_OneBranch(Node0,iLeafNode,rtp_range)
        ...
        real(8),intent(inout)       ::  rtp_range(3,2)
        integer                     ::  iChild
