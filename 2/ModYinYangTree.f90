module ModYinYangTree
    ...
    subroutine YinYangTree_Divide(Tree,Node,rtp_if_divide)
        ...
        if (Node%if_leaf) then
            ...
            do ir=1,N_rtp(1); do it=1,N_rtp(2); do ip=1,N_rtp(3)
                ...
                rtp_range1(1,:)=&
                    rtp_range(1,1)  **  [N_rtp(1)-ir+1, N_rtp(1)-ir]/N_rtp(1) * &
                    rtp_range(1,2)  **  [         ir-1,          ir]/N_rtp(1)

                rtp_range1(2,:)=&
                    rtp_range(2,1)  *   [N_rtp(2)-it+1, N_rtp(2)-it]/N_rtp(2) - &
                    rtp_range(2,2)  *   [        -it+1,         -it]/N_rtp(2)

                rtp_range1(3,:)=&
                    rtp_range(3,1)  *   [N_rtp(3)-ip+1, N_rtp(3)-ip]/N_rtp(3) - &
                    rtp_range(3,2)  *   [        -ip+1,         -ip]/N_rtp(3)
    ...
    subroutine YinYangTree_SetAll(Tree,nr,nt,np,ng,MpiSize,MpiRank)
        implicit none
        type(YYTree)                ::  Tree                ! Tree
        integer,intent(in)          ::  nr,nt,np,ng         ! grid
        ...
        do iLocalBlock=1,size(iLocalLeafNodes)
            ...
            if (Tree%iBlocksGlobal(iLocalBlock).le.Tree%NumLeafNodes_YinYang(1)) then
                if_yin=.true.
            else
                if_yin=.false.
            end if
            ...
            call ModBlock_Init(Tree%LocalBlocks(iLocalBlock),Tree%iBlocksGlobal(iLocalBlock),&
                rtp_ranges(:,:,iLocalBlock),5,nr,nt,np,ng,if_yin,if_ssm=.true.)
