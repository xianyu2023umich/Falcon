    ...
    subroutine ModCommunication_SendRecvGCAll(Tree,if_rk,MpiSize,MpiRank)
        ...
        do iLocalBlock=1,Tree%nLocalBlocks
            ...
            do iGC_source=1,size(Block1%GC_sources)
                ...
                !print *,iGC_source,size(Block1%GC_sources),allocated(GC_source1%primitive_list)
                !print *,iGC_source,size(Block1%GC_sources),allocated(GC_source1%xijk_list)
                !print *,iGC_source,size(Block1%GC_sources),allocated(Block1%primitive)
                !print *,iGC_source,size(Block1%GC_sources),allocated(Block1%primitive_rk)
                !print *,size(GC_source1%xijk_list(:,1)),size(GC_source1%xijk_list(1,:))
                !print *,size(GC_source1%primitive_list(:,1)),size(GC_source1%primitive_list(1,:))
                !print *,maxval(GC_source1%xijk_list(:,1)),maxval(GC_source1%xijk_list(:,2)),maxval(GC_source1%xijk_list(:,3))
                !print *,minval(GC_source1%xijk_list(:,1)),minval(GC_source1%xijk_list(:,2)),minval(GC_source1%xijk_list(:,3))
                !print *,maxval(Block1%xi),maxval(Block1%xj),maxval(Block1%xk)
                !print *,minval(Block1%xi),minval(Block1%xj),minval(Block1%xk)
                !print *,Block1%iBlock,GC_source1%iBlock
        ...
        do iLocalBlock=1,Tree%nLocalBlocks
            ...
            do iGC_target=1,size(Block1%GC_targets)
                if (Block1%GC_targets(iGC_target)%iRank.ne.MpiRank) then
                    ...
                    allocate(recv_message(GC_target1%nGC,Block1%nvar))
