Module ModGC
        ...
        do iLocalBlock=1,size(Tree%LocalBlocks)
            ...
            do iGC_Target=1,size(Block_target%GC_targets)
                if (Block_target%GC_targets(iGC_Target)%iRank==MpiRank) then
                    ...
                    allocate(primitive_GC(GC_target1%nGC,Block_target%nvar))
                    ...
                    if (Block_target%if_yin .eqv. Block_source%if_yin) then
                        call ModMath_1D3D_interpolate_1D1D(primitive_send,Block_source%nvar,&
                            Block_source%ni,Block_source%nj,Block_source%nk,Block_source%ng,&
    ...
    subroutine ModGC_FindGC_Single(Tree,Block1)
        ...
        integer                         ::  i,j,k,ni,nj,nk,ng       ! grid
        ...
        ! Get grid info
        ni=Block1%ni; nj=Block1%nj; nk=Block1%nk; ng=Block1%ng
    ...
    recursive subroutine ModGC_CountNeighbours(Tree,CurrentNode,Block1,NumNeighbours)
        ...
        if (CurrentNode%if_leaf) then
            ...
            dxi_CurrentNode=(CurrentNode%rtp_range(1,2)-CurrentNode%rtp_range(1,1))/Block1%ni
            dxj_CurrentNode=(CurrentNode%rtp_range(2,2)-CurrentNode%rtp_range(2,1))/Block1%nj
            dxk_CurrentNode=(CurrentNode%rtp_range(3,2)-CurrentNode%rtp_range(3,1))/Block1%nk

            ! The ranges CONSIDERING GCs
            rtp_range_GC_CurrentNode(1,1)=CurrentNode%rtp_range(1,1)-(Block1%ng-0.5)*dxi_CurrentNode
            rtp_range_GC_CurrentNode(1,2)=CurrentNode%rtp_range(1,2)+(Block1%ng-0.5)*dxi_CurrentNode
            rtp_range_GC_CurrentNode(2,1)=CurrentNode%rtp_range(2,1)-(Block1%ng-0.5)*dxj_CurrentNode
            rtp_range_GC_CurrentNode(2,2)=CurrentNode%rtp_range(2,2)+(Block1%ng-0.5)*dxj_CurrentNode
            rtp_range_GC_CurrentNode(3,1)=CurrentNode%rtp_range(3,1)-(Block1%ng-0.5)*dxk_CurrentNode 
            rtp_range_GC_CurrentNode(3,2)=CurrentNode%rtp_range(3,2)+(Block1%ng-0.5)*dxk_CurrentNode
    ...
    subroutine ModGC_GetGC_Sources_Single(Tree,Block1,MpiSize)
        ...
        do iNeighbour=1,NumNeighbours
            ...
            call ModBlock_InitGrid(Block2,rtp_ranges_Neighbours(iNeighbour,:,:),&
                1,Block1%ni,Block1%nj,Block1%nk,Block1%ng)
        ...
        do iNeighbour=1,NumNeighbours
            ...
            do iGC_target=1,Block2%nGC_targets
                if (Block2%GC_targets(iGC_target)%iBlock==Block1%iBlock) then
                    ...
                    allocate(GC_source1%primitive_list(GC_source1%nGC,Block1%nvar))
    ...
    subroutine ModGC_SetGC_Sources_Single(Tree,Block1,MpiSize)
        ...
        do iGC_source=1,Block1%nGC_sources
            ...
            call ModBlock_Init(Block2,GC_source1%iBlock,&
                ...
                1,Block1%ni,Block1%nj,Block1%nk,Block1%ng,GC_source1%if_yin,.false.)
            call ModGC_FindGC_Single(Tree,Block2)
            call ModGC_GetGC_targets_single(Tree,Block2,MpiSize)
