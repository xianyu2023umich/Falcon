module ModMath
    ...
    function ModMath_IfLinesInterSect(range1,range2,if_periodic,range_global) result(if_intersect)
        implicit none
        real,intent(in)     :: range1(2),range2(2),range_global(2)
        ...
        if (if_periodic) then
            drange=range_global(2)-range_global(1)
            if((range1(1)+drange-range2(1))*(range1(1)+drange-range2(2))<0.0 .or. &
               (range1(2)-drange-range2(1))*(range1(2)-drange-range2(2))<0.0 .or. &
               (range2(1)+drange-range1(1))*(range2(1)+drange-range1(2))<0.0 .or. &
               (range2(2)-drange-range1(1))*(range2(2)-drange-range1(2))<0.0) if_intersect=.true.
        end if
    end function

    function ModMath_IfBlocksInterSect(xijk_range1,xijk_range2,&
            ijk_if_periodic,xijk_range_global) result(if_intersect)
        implicit none
        real,intent(in)     :: xijk_range1(3,2),xijk_range2(3,2)    ! the xijk_ranges of two blocks
        real,intent(in)     :: xijk_range_global(3,2)               ! range of the whole tree
        logical,intent(in)  :: ijk_if_periodic(3)
        ...
        do direction=1,3
            if_intersect = ModMath_IfLinesInterSect(xijk_range1(direction,:),&
                xijk_range2(direction,:),ijk_if_periodic(direction),xijk_range_global(direction,:))
    ...
    subroutine ModMath_1D3D_interpolate_1D1D(f,nvar,ni,nj,nk,ng,xi,xj,xk,nout,xijk_out,fout)
        ...
        real,intent(in) :: xi(-ng+1:ni+ng),xj(-ng+1:nj+ng),xk(-ng+1:nk+ng),f(nvar,-ng+1:ni+ng,-ng+1:nj+ng,-ng+1:nk+ng)
        real,intent(in) :: xijk_out(nout,3)
